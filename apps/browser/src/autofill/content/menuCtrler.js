...
var menuCtrler = {
  ...
  }, // when frozen, you can't hide nor show the menu
};
...
const targetsEl = [], // fields where a menu has been configured
...
function _initInPageMenuForEl(targetEl) {
  ...
  const rightOffset = parseFloat(cs.paddingRight);
  targetEl.dataset.CozyRightOffset = rightOffset;
  ...
  targetEl.style.backgroundPosition =
    targetEl.clientWidth > 90
      ? `calc(100% - ${rightOffset + 3}px) 50%`
      : `calc(100% - ${rightOffset + 2}px) 1px`;
    ...
    const sameWidth = {
      ...
      fn: (pop) => {
        ...
        if (d > 0) {
          ...
          w = minMenuWidth;
          d =
            -parseFloat(pop.state.styles.popper.transform.slice(10).split("px")[0]) -
            w +
...
function _onClick(event) {
  ...
  // console.log('click 2 event in an input id:', event.target.id, event.target.clientWidth - event.target.dataset.CozyRightOffset - event.offsetX < 25);
  if (event.target.clientWidth - event.target.dataset.CozyRightOffset - event.offsetX < 25) {
    toggleShow(this);
  } else {
...
function hide(force) {
  ...
  setTimeout(() => {
    ...
    if (
      !force &&
      (targetsEl.indexOf(target) != -1 ||
        (target.tagName == "IFRAME" && target.id == "cozy-menu-in-page"))
    ) {
...
function moveSelection(n) {
  ...
  while (newCipherNode !== selectedCipher) {
    // we could not find a cipher other than the current selected one
    if (newCipherNode === null) {
